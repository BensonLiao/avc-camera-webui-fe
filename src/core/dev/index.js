...
mockAxios.onGet('/api/ping/web').reply(config => setDelay(mockResponseWithLog(config, [config.params.mock ? 500 : 200]), 10))
  ...
  .onGet('/api/multimedia/osd/settings').reply(config => mockResponseWithLog(config, [200, db.get('wordSettings').value()]))
  .onPut('/api/multimedia/osd/settings').reply(config => mockResponseWithLog(config, [200, db.get('wordSettings').assign(config.data).write()]))
