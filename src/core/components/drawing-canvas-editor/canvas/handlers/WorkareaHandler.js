...
class WorkareaHandler {
  ...
  initialize() {
    ...
    this.handler.workarea = new fabric.Image(image, workareaOption);
  ...
  setLayout = layout => {
    ...
    this.handler.getObjects().forEach(obj => {
      const {id, player} = obj;
      if (id !== 'workarea') {
        const objScaleX = isFullscreen ? scaleX : 1;
        const objScaleY = isFullscreen ? scaleY : 1;
        ...
        obj.set({
          scaleX: isFullscreen ? objScaleX : 1,
          scaleY: isFullscreen ? objScaleY : 1
        });
      }
    });
    if (isResponsive) {
   ...
   * @returns
   */
  setResponsiveImage = async (source, loaded) => {
   ...
   * @returns
   */
  setImage = async (source, loaded = false) => {
    const {canvas, workarea, editable} = this.handler;
