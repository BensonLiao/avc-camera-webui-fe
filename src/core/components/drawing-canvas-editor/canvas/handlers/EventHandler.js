...
import {convertToApiLineCoordinate, convertToApiRectCoordinate} from '../../../../utils';
...
class EventHandler {
  ...
  /**
   * Convert coordinate back based on screen size 1920*1080 and update form value
   * @param {FabricEventTarget} target
   */
  updateCoordsToForm = target => {
    const isMultipleTarget = target._objects?.filter(
      obj => obj.type === 'labeledRect' ||
      obj.type === 'directionalLine'
    ).length > 1;
    const objects = isMultipleTarget ? target._objects : [target];
    objects.forEach(target => {
      if (target.type === 'directionalLine') {
        // as of fabric.js@4.3.1, we cannot simply use obj property like oCoords as new line axis,
        // so we need to do additional calculations. ref https://github.com/fabricjs/fabric.js/issues/6711
        const points = target.calcLinePoints();
        ...
        this.handler.formValues.triggerLine = this.handler.formValues.triggerLine.map(value => {
          if (value.id === target.id) {
            value.point = [
              convertToApiLineCoordinate(
                {
                  x: point1.x,
                  y: point1.y
                },
        ...
        this.handler.setFormValues(this.handler.formValues);
      }

      if (target.type === 'labeledRect') {
        this.handler.formValues.triggerArea = this.handler.formValues.triggerArea.map(value => {
          if (value.id === target.id) {
            // TODO: calculate position inside multiple targets
            value.rect = convertToApiRectCoordinate(
              {
                left: isMultipleTarget ? target.left : target.left,
                top: isMultipleTarget ? target.top : target.top,
                width: isMultipleTarget ? target.getScaledWidth() : target.getScaledWidth(),
                height: isMultipleTarget ? target.getScaledHeight() : target.getScaledHeight()
              },
              target.canvas.width,
              target.canvas.height,
              this.handler.workarea.workareaWidth,
  ...
  moved = opt => {
    const {target} = opt;
    this.updateCoordsToForm(target);
   ...
   */
  scaled = opt => {
    this.updateCoordsToForm(opt.target);
    if (!this.handler.transactionHandler.active) {
  ...
  mousedown = opt => {
    ...
    if (editable) {
      if (this.handler.prevTarget && this.handler.prevTarget.superType === 'link') {
        this.handler.prevTarget.set({stroke: this.handler.prevTarget.originStroke});
      }
      ...
      if (this.handler.interactionMode === 'selection') {
        ...
        if (target?.type === 'directionalLine' && opt?.transform?.action === 'swapDirection') {
          this.handler.drawingHandler.directionalLine.swap(event);
        }
  ...
  mousemove = opt => {
    ...
    if (!this.handler.editable && event.target) {
      ...
      } else if (event.target !== this.handler.target) {
  ...
  keydown = e => {
    ...
    if (editable) {
      if (this.handler.shortcutHandler.isQ(e)) {
        ...
        // Disable object remove on delete until canvas/form data sync complete
        // this.handler.remove();
