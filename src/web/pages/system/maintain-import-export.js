  ...
  return (
    <>
      ...
      <div className="form-group">
        ...
        <small className="form-text text-muted my-2">{i18n.t('system.deviceMaintenance.importHelper')}</small>
        <div>
          <label className="btn btn-outline-primary rounded-pill font-weight-bold px-5">
            ...
            {i18n.t('system.deviceMaintenance.selectFile')}
          </label>
          {
            file ?
              <span className="text-size-14 text-muted ml-3">{i18n.t(file.name)}</span> :
              <span className="text-size-14 text-muted ml-3">{i18n.t('system.deviceMaintenance.noFileSelected')}</span>
          }
        </div>
        <div>
          <CustomTooltip show={!file} title={i18n.t('system.deviceMaintenance.tooltip.disabledButton')}>
            <span>
              <button
